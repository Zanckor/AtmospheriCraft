...
public class BiomeEventHandler {
    ...
    public static void startWeatherEventOnChangeChunk(PlayerChangeChunkEvent e) {
        startWeatherEvent(e.getLevelChunk().getLevel(), e.getEntity().chunkPosition().getWorldPosition(), e.getEntity().chunkPosition(), 10, 3);
    }

    public static void startWeatherEvent(Level level, BlockPos chunkBlockPos, ChunkPos chunkPos, int successPercent, int successMultiplier) {
        FakeChunk fakeChunk = LocateHash.getFakeChunk(chunkPos);
        if (!fakeChunk.isAbleToGenerateWeatherEvent()) {
            ...
            SURROUNDING_CHUNKS.forEach(chunk -> {
                if (chunk != null) {
                    ...
                    if (chunk.isAbleToGenerateWeatherEvent() && CHUNK_BIOME_ENUM.equals(BIOME_ENUM)) {
            ...
            if (MCUtil.randomBoolean(successPercent * successMultiplier)) {
                ...
                SURROUNDING_CHUNKS.forEach(chunk -> {
                    if (chunk != null) {
                        startWeatherEvent(level, chunk.getBlockPos(), new ChunkPos(chunk.getBlockPos()), 20, 1);
                    }
                });
            }
        }
